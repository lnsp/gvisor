...
func (a *addressState) DecRef() {
	...
	permanent := a.mu.kind.IsPermanent()
	// a.mu must not be locked when calling into a.addressableEndpointState
	// to not violate lock ordering. See addressState.mu for more details.
	a.mu.Unlock()
	if permanent {
